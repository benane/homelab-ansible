...
- Reverse Proxy von NPM auf **Caddy** wechseln: **in Arbeit** (`roles/caddy/`). NPMs Config liegt in einer SQLite-DB (nur UI/API), nicht versionierbar. Caddy = ein getemplatetes `Caddyfile`, automatisches HTTPS, `caddy reload` ohne Downtime, Single-Binary + systemd (gleiche Form wie die anderen Rollen). Läuft bewusst auf einem **neuen Container (VMID 204)** parallel zu NPM (208), nicht als Ersatz an Ort und Stelle – Migration Hostname für Hostname per Override auf `service:` im jeweiligen `cloudflared_ingress`-Eintrag (`cloudflared_default_origin` bleibt bis zum Schluss auf NPM zeigen), NPM erst abbauen wenn alles umgezogen ist. Danach wird das `Caddyfile` aus derselben Liste generiert wie cloudflared-Ingress / Gatus / DNS → Anker für die „ein Fact, viele Konsumenten"-Kette (siehe „Self-Registration"). Traefik wäre die mächtigere, steilere Alternative (File-Provider).
  - **CrowdSec:** läuft aktuell auf dem NPM-Container, zieht nicht automatisch um. Bleibt auf demselben Host wie der Reverse Proxy (braucht dessen Logs lokal) – als OS-Firewall-Bouncer (iptables/nftables), entkoppelt vom konkreten Proxy, statt als proxy-spezifisches Plugin (das bei Caddy einen Custom-Build via `xcaddy` bräuchte). Kein fail2ban zusätzlich – deckt dieselben Fälle ab (auch SSH, via `crowdsecurity/sshd`-Collection), wäre Redundanz. Neue Erkennungs-Scenarios (z.B. für frisch ausgenutzte CVEs) werden über `cscli hub update/upgrade` gezogen, aber erst nach `systemctl restart crowdsec` aktiv – braucht also einen wiederkehrenden Update-Job, nicht nur einmalige Installation. Zentralisierte CrowdSec-Instanz (Agent auf einer Maschine, Bouncer auf mehreren) erst, wenn mehr als ein Host davon profitiert – gleiche Regel wie bei „Self-Registration": lohnt sich erst ab mehreren Konsumenten.
