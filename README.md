...
- Reverse Proxy von NPM auf **Caddy** wechseln: **Dienste umgezogen (2026-09-20), NPM-Abbau offen** (`roles/caddy/`). NPMs Config liegt in einer SQLite-DB (nur UI/API), nicht versionierbar. Caddy = ein getemplatetes `Caddyfile`, automatisches HTTPS, `caddy reload` ohne Downtime, Single-Binary + systemd (gleiche Form wie die anderen Rollen). Läuft auf einem **eigenen Container (VMID 204)**. Alle sechs Tunnel-Dienste (`paperless`, `wattwarriors`, `auth`, `immich`, `seer`, `status`) sind über die Service-Registry auf Caddy umgezogen (`cloudflared_default_origin` zeigt jetzt auf Caddy statt NPM), dazu die zuvor schon migrierten `intern`-Dienste (Gatus, Zigbee2MQTT, Grafana, VictoriaMetrics, cloudflared-a/-b, Caddy selbst). NPM (208) läuft noch parallel weiter, bekommt aber keinen Traffic mehr für diese Namen – Abbau (LXC stoppen, danach löschen, README-Punkt schließen) folgt nach ein paar Tagen Beobachtung, siehe Plan Phase 13. Traefik wäre die mächtigere, steilere Alternative gewesen (File-Provider).
